...
int a,b,c,cell,chain,length,chain_length[2080];
long int natom=0;
...
int main()
{
        ...
        for(int i=0;i<chain_length[n];i++)
        {
            ...
            else 
            {
                ...
                do{
                    ...
                    if((i+1)%8==3||(i+1)%8==7)//判定是否为苯环的中心虚拟原子
                    {
                    ...
                    if(xx<=0)xx+=a;
                    if(yy>=b)yy-=b;
                    if(yy<=0)yy+=b;//zz,xx,yy在复制的父节点基础上走动一格。
    ...
    for (int n=0;n<chain;n++)
    {
        for(int i=0;i<chain_length[n];i++)
        {
            ...
            {
                pe[n][i].type=3;  //苯环的虚拟碳
            }
            else pe[n][i].type=4; //羰基碳
    ...
    for(int n=0;n<chain;n++)
    {
        ...
        {
            num++;
            cout<<num<<"    "<<n+1<<"    "<<pe[n][i].type<<"   "<<realpe[n][i].x*1.082<<"    "<<realpe[n][i].y*1.082<<"    "<<realpe[n][i].z*1.082<<endl;
  ...
  // //-----------生成链的周期性边界位置---------
    ...
    //     {
    //         num++;
    //         cout<<num<<"    "<<n+1<<"    "<<pe[n][i].type<<"   "<<pe[n][i].x*1.082<<"    "<<pe[n][i].y*1.082<<"    "<<pe[n][i].z*1.082<<endl;
